...
public:
    BloomFilter(size_t hashes, size_t bits) {
        ...
        hashes_ = hashes;
        num_of_bits_ = bits;
        table_ = new HashTable<std::string, std::string>();
        fprate_ = 0;
        verify_cnt_ = 0;
    }
    ...
    void add(const std::string &obj) {
        u_long index = std::hash<std::string>{}(obj) % num_of_bits_;
        bits_[index] = true;
        for (size_t i = 1; i < hashes_; ++i) {
            index = std::hash<std::string>{}(obj + std::to_string(i)) % num_of_bits_;
            bits_[index] = true;
        }
        table_->insert(obj, obj);
    }
    ...
    bool verify(const std::string &obj) {
        ...
        if (table_->find(obj) == nullptr && res) {
            fprate_++;
        }
        verify_cnt_++;
        return res;
    ...
    double getFPRate() const {
        ...
        return static_cast<double>(fprate_) / verify_cnt_;
    }
